    ...
    public AluraServiceStack(final Construct scope, final String id, final StackProps props, final Cluster cluster) {
        ...
        scalableTarget.scaleOnCpuUtilization("CpuScaling", CpuUtilizationScalingProps.builder()
                .targetUtilizationPercent(70)
                        .scaleInCooldown(Duration.minutes(3)) //define qto tempo a cpu deve estar com 70% até subir uma nova instância
                        .scaleOutCooldown(Duration.minutes(2)) //define qto tempo a cpu deve estar abaixo de 70% para derrubar uma instância
                .build());

        scalableTarget.scaleOnMemoryUtilization("MemoryScaling", MemoryUtilizationScalingProps.builder()
                .targetUtilizationPercent(65)
                    .scaleInCooldown(Duration.minutes(3)) //define qto tempo a memoria deve estar com 65% até subir uma nova instância
                    .scaleOutCooldown(Duration.minutes(2)) //define qto tempo a memoria estar abaixo de 65% para derrubar uma instância
                .build());
    }
}
